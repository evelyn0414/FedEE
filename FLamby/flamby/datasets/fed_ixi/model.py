...
def getallfea(self, x):
    ...
    #         i += 1
    #         fea = layer(fea)
    #     elif name.startswith("MaxPool3d"):
    #         skip_connections.append(fea)
    #         fea = layer(fea)
    #     elif name.startswith("DecodingBlock"):
    #         fea = layer.upsample(fea)
    #         skip_connection = layer.center_crop(skip_connections[-1], fea)
    #         skip_connections.pop()
    #         print(skip_connection.shape)
    #         print(fea.shape)
    #         fea = torch.cat((skip_connection, fea), dim=CHANNELS_DIMENSION)
